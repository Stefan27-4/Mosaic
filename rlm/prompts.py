...
# Standard RLM system prompt with full sub-call capabilities
...
# Conservative prompt that discourages excessive sub-calls (for models like Qwen)
RLM_SYSTEM_PROMPT_CONSERVATIVE = """You are tasked with answering a query with associated context. You can access, transform, and analyze this context interactively in a REPL environment that can recursively query sub-LLMs, which you are strongly encouraged to use as much as possible. You will be queried iteratively until you provide a final answer.
...
# Ablation prompt without sub-call capability
